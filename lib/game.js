...
class Game {
  ...
  processClickSelection(clickX, clickY) {
    if (this.mmState != Util.menuStates.GAME) {
      ...
      if (this.mmState == Util.menuStates.MAIN) {
        ...
        this.handleScoreMenuClickEvent(canvasX, canvasY, rect);
   ...
   * @param {any} clickY
   */
  handleScoreMenuClickEvent(clickX, clickY, rect) {
    var minButtonColumn = Math.floor(parseInt(rect.width / 3));
    var maxButtonColumn = Math.floor(parseInt(rect.width * 2 / 3));  
    var leftButtonColumn = Math.floor(parseInt(minButtonColumn + (maxButtonColumn - minButtonColumn) / 3));
    var middleButtonColumn = Math.floor(parseInt(minButtonColumn + (maxButtonColumn - minButtonColumn) / 1.5));
    var rightButtonColumn = Math.floor(parseInt(minButtonColumn + ((maxButtonColumn - minButtonColumn) * 2 / 3)));
      

    //console.log("(" + clickX + ", " + clickY + ") and X columns: " + minButtonColumn + ", " + leftButtonColumn + ", " + middleButtonColumn + ", " + rightButtonColumn + ", " + maxButtonColumn);
    //console.log("Y of plus row min: " + INITIAL_PLUS_MIN_POSY);
    //console.log("Y of minus row min: " + INITIAL_MINUS_MIN_POSY);
    //console.log("Y of submit row min: " + SUBMIT_SCORE_MIN_POSY);

    if (clickX > minButtonColumn && clickX <= leftButtonColumn && clickY >= INITIAL_PLUS_MIN_POSY && clickY <= INITIAL_PLUS_MAX_POSY) {
      // Handle first initial plus button click...
      this.initial1 = this.getNextLetterPosition(this.initial1);
    } else if (clickX > leftButtonColumn && clickX <= middleButtonColumn && clickY >= INITIAL_PLUS_MIN_POSY && clickY <= INITIAL_PLUS_MAX_POSY) {
      // Handle  middle initial plus button click...
      this.initial2 = this.getNextLetterPosition(this.initial2);
    } else if (clickX > middleButtonColumn && clickX < maxButtonColumn && clickY >= INITIAL_PLUS_MIN_POSY && clickY <= INITIAL_PLUS_MAX_POSY) {
      // Handle right initial plus button click...
      this.initial3 = this.getNextLetterPosition(this.initial3);
    } else if (clickX > minButtonColumn && clickX <= leftButtonColumn && clickY >= INITIAL_MINUS_MIN_POSY && clickY <= INITIAL_MINUS_MAX_POSY) {
      // Handle left initial minus button click...
      this.initial1 = this.getPreviousLetterPosition(this.initial1);
    } else if (clickX > leftButtonColumn && clickX <= middleButtonColumn && clickY >= INITIAL_MINUS_MIN_POSY && clickY <= INITIAL_MINUS_MAX_POSY) {
      // Handle middle initial minus button click...
      this.initial2 = this.getPreviousLetterPosition(this.initial2);
    } else if (clickX > middleButtonColumn && clickX < maxButtonColumn && clickY >= INITIAL_MINUS_MIN_POSY && clickY <= INITIAL_MINUS_MAX_POSY) {
      // Handle right initial minus button click...
      this.initial3 = this.getPreviousLetterPosition(this.initial3);
    } else if (clickX >= minButtonColumn && clickX <= maxButtonColumn && clickY >= SUBMIT_SCORE_MIN_POSY && clickY <= SUBMIT_SCORE_MAX_POSY) {
  ...
  getUpdatedScores() {
    ...
    scoreClient.httpGetAsync("http://localhost:5000/scores/", 10000, String(this.difficulty),
  ...
  submitScore() {
    ...
    scoreClient.httpPostAsync("http://localhost:5000/scores/new/", 10000, sInitials, this.scores.score, String(this.difficulty),
      (function callbackCloser(test) {
        //console.log(test);
        return callback;
      })(this.scores.score)
  ...
  renderScoreEntryScreen(ctx) {
    ...
    ctx.fillText("+" + "   " + "+" + "   " + "+", 126, 350);
    ctx.fillText(SELECTABLE_LETTERS[this.initial1] + "   " + SELECTABLE_LETTERS[this.initial2] + "   " + SELECTABLE_LETTERS[this.initial3], 126, 400);
    ctx.fillText("_" + "   " + "_" + "   " + "_", 126, 430);
  ...
  renderGameOverScreen(ctx) {
    ...
    ctx.fillStyle = 'white';
    ctx.fillText("(" + this.getDifficultyString() + ")", 72, 150);
